# UTags - Ajouter des tags utilisateur aux liens
...
- Et bien d'autres. Cliquez [ici](https://github.com/utags/utags/issues) pour suggérer de nouveaux sites.

Si vous souhaitez utiliser UTags sur d'autres sites, ajoutez simplement une règle `user matches` pour le site cible dans les paramètres du script.
...
- 0.23
  - Optimisation de la logique de repli (fallback) de l'API GM.\*.
  - Amélioration de la compatibilité avec les gestionnaires de scripts tels que Greasymonkey, quoid-userscripts, Stay, etc.
  - Amélioration de l'interaction clavier pour les touches Échap et Tab.
  - Correction du problème Safari où la fermeture du panneau de saisie des tags faisait défiler la page vers le bas.
- 0.22
  - Suppression de `// @noframes` et activation de `all_frames: true` pour permettre à UTags de s'exécuter dans les iframes, améliorant la compatibilité avec l'extension utags-shortcuts et les userscripts.
  - Mise à jour du module de paramètres.
- 0.21
  - Optimiser la gestion des anomalies de version distante : après confirmation, effectuer une première fusion (`lastSyncTime=0`, privilégier les données les plus récentes) ou annuler pour désactiver la synchronisation.
  - Nouveau commande de menu : Masquer/Afficher toutes les tags avec titre dynamique et support multilingue.
- 0.20
  ...
  - Ajouter de nouveaux tags spéciaux commençant par des points d'exclamation : !, !!, !!!, !important pour le marquage de priorité
- 0.19.x
...
- 0.9.8
  - twitter.com -> x.com
  - Correction des problèmes avec github.com, threads.net
- 0.9.5
  - Ajout de tags emoji👍
...
- 0.8.9
  - Mise à jour des styles et règles de correspondance pour bilibili.com, greasyfork.org, youtube.com, douyin.com
- 0.8.8
